...
function checkRefreshToken(refreshToken, name) {
  // checking user refreshToken for repairing both tokens
  return userSchema.find({ $and: [{ refreshToken: refreshToken }, { name: name }] })
    .then(function (response) {
      if (response.status == "ok") {
        if (response.data === null) {
          logger.log("info", "[User][checkRefreshToken] not found: refreshToken=%s name=%s", refreshToken, name);
          return { status: "error", code: 60, message: "Not found users" };
        } else {
